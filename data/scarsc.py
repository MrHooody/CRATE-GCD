...
class MergedCarscDataset(Dataset):
    ...
    def __init__(self, data_dir, ori_dir, train=True, limit=0, transform=None, metas=scars_meta_path): # remove args
...
def get_scarsc_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
    ...
    if args.task_type == 'A_L+A_U->B':
        ...
        whole_training_set = CarsDataset(data_dir=scars_root, transform=train_transform, metas=scars_meta_path, train=True)
        train_datasetB_unlabelled = MergedCarscDataset(data_dir=scarsc_root, ori_dir=scars_root, transform=train_transform, metas=scars_meta_path, train=False)
        test_datasetB = MergedCarscDataset(data_dir=scarsc_root, ori_dir=scars_root, transform=test_transform, metas=scars_meta_path, train=False)
