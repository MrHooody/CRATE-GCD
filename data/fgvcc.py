...
class MergedFGVCcAircraft(Dataset):
    ...
    def __init__(self, root, root_ori, class_type='variant', split='train', transform=None,
                 target_transform=None, loader=default_loader, download=False): # args will conflict with init_experiment
        if split not in self.splits:
            raise ValueError('Split "{}" not found. Valid splits are: {}'.format(
                split, ', '.join(self.splits),
            ))
    ...
    def __getitem__(self, index):
        ...
        ori_path, target = self.samples[sample_idx]
...
def get_fgvcc_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
    ...
    if args.task_type == 'A_L+A_U->B':
        ...
        # Init entire training set
        whole_training_set = FGVCAircraft(root=fgvc_root, transform=train_transform, split='trainval')
        train_datasetB_unlabelled = MergedFGVCcAircraft(root=fgvcc_root, root_ori=fgvc_root, transform=train_transform, split='test')
        test_datasetB = MergedFGVCcAircraft(root=fgvcc_root, root_ori=fgvc_root, transform=test_transform, split='test')
